...
import scanpy as sc
import toolz
from crukiopy.gcsutils import download_from_gs
from sctools.scplotting import extract_cmap_from_adata, extract_cmap_from_h5ad
...
#         if True:
#             tmpfolder = '/home/mstrasse/TB4/tmp'
            if self.is_remote():
                #download to tempdir
            ...
            # also extract the colors from the adata
            # note: do this on the postprocess data, colors might have been adjusted!

            # todo: pretyy silly to load the entire thing here!
            # maybe we can pull it out from disc
            if False:
                a = sc.read_h5ad(new_h5_filename)

                cat_fields = [field for field in a.obs.columns if a.obs[field].dtype == "category" and f'{field}_colors' in a.uns]
                cmap = {}
                for field in cat_fields:
                    cmap_tmp =  extract_cmap_from_adata(a, field)
                    cmap.update(cmap_tmp)
                del a
                import gc
                gc.collect()
            else:
                # this extracts the colormaps OUT OF MEMORY!!
                obs = load_obs(new_h5_filename)
                with h5py.File(new_h5_filename, 'r') as f:
                    uns_fields = list(f['/uns'].keys())  # need to pull it into python while the handle is open
                cat_fields = [field for field in obs.columns if obs[field].dtype == "category" and f'{field}_colors' in uns_fields]
                cmap = {}
                for field in cat_fields:
                    cmap_tmp =  extract_cmap_from_h5ad(new_h5_filename, field)
                    cmap.update(cmap_tmp)
        ...
        self._write_colors(dataset_dir, cmap)
    ...
    def _write_colors(self, directory, cmap:dict):
        with open(f'{directory}/colors.csv', 'w') as fh:
...
# bucket_to_list('gs://cruk-adata-for-cellbrowser/March2021_adatas/custom')
...
def cellbrowser_to_webpage(cellbrowser_folder:str, out_folder: str ):
    """
    ...
    target = pathlib.Path(out_folder)
